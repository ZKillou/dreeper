...
export class DreeperCommandInteraction {
  ...
  async buildOptions() {
    this.options = await DreeperCommandInteractionOptionResolver.create(
      this.client,
      this
    );
  }
  ...
  async reply(
    options: CommandReplyOption | string
  ): Promise<Message | undefined | null> {
    if (typeof options === 'string') options = { content: options };
    if (
      !(this.class instanceof Message) &&
      (this.class.replied || this.class.deferred)
    ) {
      // @ts-ignore
  ...
  async editReply(
    ...
    if (!this.replyMessage)
      throw new SyntaxError('The reply must be sent before editing it.');
    if (this.class instanceof Message) {
...
export class DreeperCommandInteractionOptionResolver {
  ...
  static async create(
    ...
    let options = await DreeperCommandInteractionOptionResolver.generateOptions(
      interaction
    );
    ...
    if (!optionsdata)
      throw TypeError(
        'Invalid data provided to build DreeperCommandInteractionOptionResolver'
      );
  ...
  static async generateOptions(
    ...
    if (
      interaction.class instanceof Message &&
      interaction.client.config.prefix
    ) {
      const { content } = interaction.class;
      let args = content
        .slice(interaction.client.config?.prefix.length)
        .split(/ +/);
      args.shift();
      ...
      if (hoisted[0]?.type === 'SUB_COMMAND_GROUP') {
        ...
        if (!args[i])
          return {
            error: true,
            text: 'INVALID_SUBCOMMANDGROUP',
            intended: subcommandgrouplist,
            given: 'none',
          };

        let subcommandgroup:
          | ApplicationCommandSubGroupData
          | undefined = hoisted.find(
          o => o.name === args[i].toLowerCase()
        ) as any;
        ...
        let subcommand = await DreeperCommandInteractionOptionResolver._extractSubCommand(
          { hoisted, args, interaction },
          i
        );
      ...
      let options = await DreeperCommandInteractionOptionResolver._extract({
        hoisted,
        args,
        interaction,
      });
      ...
      throw new TypeError(
        'Unknown class used to build a DreeperCommandInteractionOptionResolver'
      );
    }
  }

  static async _extractSubCommand(
    data: extractOptions,
    i = 0
    ...
    if (!data.args[i])
      return {
        error: true,
        text: 'INVALID_SUBCOMMAND',
        intended: subcommandlist,
        given: 'none',
      };

    let subcommand:
      | ApplicationCommandSubCommandData
      | undefined = data.hoisted.find(
      o => o.name === data.args[i].toLowerCase()
    ) as any;
    ...
    return {
      result: [
        { type: 'SUB_COMMAND', name: subcommand.name, options: optionsdata },
      ],
    };
  }
  ...
  static async _extract(data: extractOptions, i = 0): Promise<extractResult> {
    ...
    for (let j = 0; j < data.hoisted.length; j++) {
      ...
      if (commandoption.type === 'NUMBER' || commandoption.type === 'INTEGER') {
        value = parseInt(arg);
        if (isNaN(value))
          return { error: true, text: 'INVALID_NUMBER', given: arg };
        if (commandoption.minValue)
          if (value < commandoption.minValue)
            return {
              error: true,
              text: 'INVALID_NUMBER_SMALL',
              given: arg,
              intended: commandoption.minValue,
            };
        if (commandoption.maxValue)
          if (value < commandoption.maxValue)
            return {
              error: true,
              text: 'INVALID_NUMBER_BIG',
              given: arg,
              intended: commandoption.maxValue,
            };
      } else if (commandoption.type === 'BOOLEAN') {
        if (arg.toLowerCase() !== 'true' && arg.toLowerCase() !== 'false')
          return { error: true, text: 'INVALID_BOOLEAN', given: arg };
        ...
        const matches = arg.matchAll(MessageMentions.USERS_PATTERN).next()
          .value;
        if (!matches) return { error: true, text: 'UNKNOWN_USER' };
        const user = await data.interaction.client.users
          .fetch(matches[1])
          .catch();
        const guildmember = await data.interaction.guild?.members
          .fetch(matches[1])
          .catch();
        ...
        const matches = arg.matchAll(MessageMentions.ROLES_PATTERN).next()
          .value;
        if (!matches) return { error: true, text: 'UNKNOWN_ROLE' };
        const role = await data.interaction.guild?.roles
          .fetch(matches[1])
          .catch();
        if (!role) return { error: true, text: 'UNKNOWN_ROLE' };
        value = role;
      } else if (commandoption.type === 'CHANNEL') {
        const matches = arg.matchAll(MessageMentions.CHANNELS_PATTERN).next()
          .value;
        if (!matches) return { error: true, text: 'UNKNOWN_CHANNEL' };
        const channel = await data.interaction.guild?.channels
          .fetch(matches[1])
          .catch();
      ...
      if (commandoption.choices) {
        ...
        if (!selected)
          return {
            error: true,
            text: 'ARG_CHOICE_INVALID',
            given: arg,
            intended: commandoption.choices.map(c => c.name).join(', '),
          };
        else value = selected;
      }
  ...
  get(
    name: string,
    required = false
  ): ResolvedOption | CommandInteractionOption | null {
  ...
  _getTypedOption(
    ...
    if (!option) {
      ...
      throw new TypeError(
        `COMMAND_INTERACTION_OPTION_TYPE ${name} ${option.type} ${type}`
      );
    }
    return option;
  }
  ...
  getChannel(
    name: string,
    required = false
  ): Channel | TextChannel | VoiceChannel | CategoryChannel | null {
