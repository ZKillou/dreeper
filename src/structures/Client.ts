...
export class DreeperClient extends Client {
  ...
  constructor(djsoptions: ClientOptions, dreeperoptions: DreeperOptions) {
    ...
    if (dreeperoptions.config instanceof Config)
      this.config = dreeperoptions.config;
  ...
  async init() {
    ...
    if (this.doptions.dirs.commands)
      loadCommands(this, this.doptions.dirs.commands);
